...
void displayWelcomeBanner(uuid::console::Shell &shell) {
    shell.println(F("+------------------------------------------+"));
    shell.println(F("|       \033[32mLibreLinkUp ESP32 Console\033[0m          |"));
    if (fw_update_is_update_available()) {
        char fw_str[48];
        // \033[33m (5 bytes) + \033[0m (4 bytes) = 9 invisible bytes; widen field to compensate
        snprintf(fw_str, sizeof(fw_str), "%s \033[33m-> %s\033[0m",
                 fw_update_get_current_version(), fw_update_get_latest_version());
        shell.printfln(F("|  FW : %-44s|"), fw_str);
    } else {
